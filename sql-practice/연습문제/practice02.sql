...
select max(salary) as '최고 연봉', min(salary) as '최저 임금'
  from salaries;

-- 문제6.
-- 최고 어린 사원의 나이와 최 연장자의 나이는?
select (substring(now(), 1, 4) - substring(max(birth_date), 1,4)) as '최연소', (substring(now(), 1, 4) - substring(min(birth_date), 1,4)) as '최연장자'
  from employees;
